# CS5281 Internet Application Development

> 香港城市大学 CS 硕士 · Semester B 2025/26

- **课程代码**：CS5281
- **课程名称**：Internet Application Development
- **学分**：3
- **状态**：🟡 已有课程材料

## 主要内容

- Web 应用基础：HTTP/HTTPS、客户端-服务器架构、浏览器渲染
- 前端开发：HTML/CSS/JavaScript、响应式设计、主流框架基础
- 后端开发：服务器端语言、RESTful API 设计、路由、模板引擎
- 数据库集成：关系型数据库、ORM/ODM、会话管理
- Web 安全、认证授权、部署与运维基础
- 全栈项目实践

## 课程材料（Course Materials）

> **版权说明**：`materials/official/` 收录 CityU 课程门户可下载的官方课件（讲义、Tutorial、作业说明、试题等，不含答案），版权归原作者及 CityU 所有，本人不主张任何版权；若版权方认为不妥可随时联系撤下。`materials/my-notes/` 为本人原创与从课程社群收集的学习整理，遵循 MIT 协议开放。

**官方课件 `official/`（不含答案）**
- `official/lecture/` —— 课程讲义 slides：`02HTML.pdf`、`combined_4up.pdf`
- `official/tutorial/` —— Tutorial 练习（T1–T5）：HTML / Form / Inline & External CSS / JavaScript Form
- `official/revision/` —— `Revision+Exercise+Que.pdf` 复习练习题

**个人与社群笔记 `my-notes/`（MIT 开放）**
- [practice_knowledge_summary.pdf](materials/my-notes/practice_knowledge_summary.pdf)（知识总结）
- [tutorial1-4_exam_guide_annotated.pdf](materials/my-notes/tutorial1-4_exam_guide_annotated.pdf)（带标注考试指南）

## 笔记索引

| 序号 | 主题 | 链接 |
|---|---|---|
| 01 | 练习知识总结（Practice Knowledge Summary）| [practice_knowledge_summary.pdf](materials/my-notes/practice_knowledge_summary.pdf) |
| 02 | Tutorial 1–4 考试指南（带标注）| [tutorial1-4_exam_guide_annotated.pdf](materials/my-notes/tutorial1-4_exam_guide_annotated.pdf) |

> 原创学习整理，遵循 MIT 协议开放。结构参考 [`templates/note-template.md`](../../templates/note-template.md)。

## 参考资料
- 课程官方材料（请从 CityU 课程门户获取）
- 教材（待补充）
